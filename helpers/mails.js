...
const constructActivationEmail = user => {
    ...
    const path = `${process.env.SERVER_ENDPOINT || endpoint}/activate-email?key=${activate_or_reset_pass_key}`;
...
const constructForgotCredentialsEmail = (user, key) => {
    ...
    const path = `${process.env.SERVER_ENDPOINT || endpoint}/reset?key=${key}`;
    let newEmail = { ...rawEmail };
    newEmail.to = email;
